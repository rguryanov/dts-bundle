...
function bundle(options) {
    ...
    var typingsDir = path.resolve(optValue(options.typingsDir, 'typings'));
	...
	var excludedTypings = [];
    var usedTypings = [];
    var getExternalMap = function () {
        var getFilesSync = function (dir) {
            var results = [];
            ...
            for (var i = 0; i < list.length; i++) {
                ...
                if (stat && stat.isDirectory()) {
                    ...
                    if (isExclude(file, true)) {
						trace(' - exclude external module typing with file %s', file);
						pushUnique(excludedTypings, file);
						return;
					} else {
						results.push(file);
					}
        ...
        var fileMap = Object.create(null);
        var files = getFilesSync(typingsDir);
        ...
        var exportsMap = Object.create(null);
        Object.keys(fileMap).forEach(function (file) {
            var parse = fileMap[file];
            parse.exports.forEach(function (name) {
    ...
    var exportTypingMap = getExternalMap();
